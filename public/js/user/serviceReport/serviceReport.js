...
async function addNewPage(index) {
    ...
    for (let i = index; i < services.length; i++) {
        $(`.all-machines-table-tbody-${pages}`).append(`
            <tr data-row-index=${i} data-table-no=${pages} class="table-row-${pages}">
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden">
                    <span style="word-break:break-word;border:none;width:100%;">${services[i].machineName}<br>${services[i].machineNum}</span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden">
                    <span style="word-break:break-word;border:none;width:3em;">${services[i].warranty.trim()}</span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden">
                    <span style="word-break:break-word;border:none;width:100%;">${services[i].problem.trim()}</span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:18em;">
                    <span style="word-break:break-word;border:none;width:100%;">${services[i].actionTaken.trim()}</span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:7em">
                    <span style="word-break:break-word;border:none;width:100%;" class="parts-in-part-name-${i}"></span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:7em">
                    <span type="text" style="word-break:break-word;border:none;width:100%;" class="parts-in-part-out-${i}"></span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:7em">
                    <span style="word-break:break-word;border:none;width:100%;" class="parts-in-part-wty-${i}"></span>
                </td>
                <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:5em">
                    <span style="word-break:break-word;border:none;width:100%;">${services[i].status.trim()}</span>
                </td>
            </tr>`)
        if (!(0 in services[i].partsIN[0])) {
            ...
            $(`.parts-in-part-name-${i}`).val(services[i].partsIN[0].partSerialNumber)
            $(`.parts-in-part-wty-${i}`).val(services[i].partsIN[0].partWty)
            $(`.parts-in-part-out-${i}`).val(services[i].partsOUT[0].partName)
            for (let j = 1; j < services[i].partsIN.length; j++) {
                $(`.all-machines-table-tbody-${pages}`).append(`
                <tr data-row-parts="part-row-${i}">
                    <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:7em;">
                        <span style="word-break:break-word;border:none;width:100%;">${services[i].partsIN[j].partSerialNumber}<br></span>
                    </td>
                    <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:7em;">
                        <span style="word-break:break-word;border:none;width:100%;">${(services[i].partsOUT[j] != undefined) ? services[i].partsOUT[j].partName : ''}<br></span>
                    </td>
                    <td style="word-break:break-all;font-size:12px;overflow-y:hidden;width:7em;">
                        <span style="word-break:break-word;border:none;width:100%;">${services[i].partsIN[j].partWty}<br></span>
                    </td>
                </tr>`)
            ...
            $(`.parts-in-part-name-${i}`).html(partInInputsArr.replaceAll(',', '<br>'))
            $(`.parts-in-part-out-${i}`).html(partOutInputsArr.replaceAll(',', '<br>'))
            $(`.parts-in-part-wty-${i}`).html(services[i].partWarranty.replaceAll(' ?>> ', '<br>'))
        }
