...
function viewServiceReport(report) {
  ...
  $(".revisions form").attr(
    "onsubmit",
    `updateCheckStatus('${report._id}',$(this),'tally');return false`
  );
  if (report.checked.tally.isReviewed) {
    $('.revision-tally form div:nth-child(1) input[type="checkbox"]').prop(
      "checked",
      true
    );
    $('.revision-tally form input[name="bookNo"]').val(
      report.checked.tally.bookNo
    );
    $('.revision-tally form input[name="bookDate"]').val(
      report.checked.tally.bookDate
    );
    setRevisedTally(true);
  } else {
    setRevisedTally(false);
  }
  ...
  $(".service-report-details-data-time").val(
    `${report.date.split("/")[1]}/${report.date.split("/")[0]}/${
      report.date.split("/")[2]
    } ----- ${report.time}`
  );
  ...
  if (reportLength != 0) {
    ...
    for (let i = 0; i < reportLength; i++) {
      data += `
                <tr class='cursor-pointer service-report-row' onclick='viewMachineDetails(${JSON.stringify(
                  report.service[i]
                )});toggleServiceReportInputs("machine",true);$(".submit-changes-service-report-machine-details-btn").addClass("hide");$(".change-service-report-machine-details-btn").removeClass("hide")'>
                <td data-label="Machine Name" >${ShortifyString(
                  report.service[i].machineName
                )}</td>
                <td data-label="Machine Number" >${ShortifyString(
                  report.service[i].machineNum
                )}</td>
                <td data-label="Warranty" >${ShortifyString(
                  report.service[i].warranty
                )}</td>
                <td data-label="Problem" >${ShortifyString(
                  report.service[i].problem
                )}</td>
                <td data-label="Action" >${ShortifyString(
                  report.service[i].actionTaken
                )}</td>
                <td data-label="Parts IN" >${
                  report.service[i].partsIN.length
                }</td>
                <td data-label="Parts OUT">${
                  report.service[i].partsOUT.length
                }</td>
                <td data-label="Status" >${ShortifyString(
                  report.service[i].status
                )}</td>
                </tr>`;
    }
    $("#all-service-machines-table-body").html(data);
  }
  $(".print-service-report-btn").attr(
    "onclick",
    `window.open('/vitco-india/control/service-reports/print/${report._id}', '_blank')`
  );
...
function viewMachineDetails(machine) {
  ...
  $(".service-report-machine-details-machine-name").val(
    `${machine.machineName}`
  );
  $(".service-report-machine-details-machine-password").val(
    `${machine.machinePassword}`
  );
  $(".service-report-machine-details-machine-number").val(
    `${machine.machineNum}`
  );
  ...
  if (machine.partsIN[0][0] == undefined) {
    ...
    partWty =
      machine.partWarranty != undefined
        ? machine.partWarranty.split(" ?>> ")
        : "No data";
      ...
      partIninputsHtml += `
            <div class="d-flex">
                <input readonly value="${
                  partin[i]
                }" type="text" class="form-control mx-1 shadow-none service-report-machine-details-parts-in partName my-1" placeholder="enter parts name">
                ...
                <input readonly value="${
                  typeof partWty == "object" ? partWty[i] : "null"
                }" type="text" class="form-control mx-1 shadow-none service-report-machine-details-parts-in partWty my-1" placeholder="enter parts wty">
            </div>`;
    }
        ...
        <tr class='cursor-pointer move service-report-row row-${i}'>
         <td class="user-select-none" data-label="Viewed" style="padding: 1em;">
            <input class="form-check-input cursor-pointer" type="checkbox" ${
              $.inArray(
                $("#admin-id").val(),
                reports[i].checked.adminViewed.admins
              ) != -1
                ? "checked"
                : ""
            } id="flexCheckDefault"
         ...
         <td class="user-select-none" data-label="Report Number"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(reports[i].reportNumber)}</td>
         <td class="user-select-none" data-label="Date"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(
      `${reports[i].date.split("/")[1]}/${reports[i].date.split("/")[0]}/${
        reports[i].date.split("/")[2]
      }`
    )}</td>
         <td class="user-select-none" data-label="Customer name"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(reports[i].customerName)}</td>
         <td class="user-select-none" data-label="Mobile number"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(reports[i].mobile)}</td>
         <td class="user-select-none" data-label="Address"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(reports[i].address)}</td>
         <td class="user-select-none" data-label="Total Machines"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(reports[i].service.length)}</td>
         <td class="user-select-none" data-label="Tech. Name"  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${ShortifyString(reports[i].technicianName)}</td>
         <td class="user-select-none" data-label="Attd. Loc."  onclick='viewServiceReport(${JSON.stringify(
           reports[i]
         )});'>${
      reports[i].checked.tally.isReviewed
...
async function getAllServiceReports() {
  ...
  await axios
    .get("/api/v1/service-report/get-report-number")
    .then((response) => {
  ...
  if (from > 20) {
    await axios
      .get(`/api/v1/service-report/all/${from}/${to}`)
      .then((response) => {
    ...
    if (gotAllreports == false) {
      await axios.get(`/api/v1/service-report/all/0/${to}`).then((response) => {
        gotAllreports = true;
        updateServiceReportTable(response.data.machines);
      });
    }
...
async function updateCheckStatus(id, r, status) {
  if (status === "viewed") {
    ...
    const checkboxSelector = $(
      `.service-report-row.row-${r} td[data-label="Viewed"] input[type="checkbox"]`
    );
    const loaderSelector = $(
      `.service-report-row.row-${r} td[data-label="Viewed"] .spinner-border`
    );
    const THcheckboxSelector = $(
      `.all-service-reports-table th:nth-child(1) input[type="checkbox"]`
    );
    const THloaderSelector = $(
      `.all-service-reports-table th:nth-child(1) .spinner-border`
    );
    ...
    await axios
      .post(`/api/v1/service-report/checked/viewed`, {
        id,
        Cid: $("#admin-id").val(),
        data,
      })
      .then((response) => {
        if (response.data.success) {
          if (id != "0") {
            ...
            $(
              `.service-report-row td[data-label="Viewed"] input[type="checkbox"]`
            ).prop("checked", state);
          }
        }
      })
    ...
    data = {};
    formData = r.serializeArray().map((v) => {
      data[v.name] = v.value;
    });
    data["isReviewed"] = $('.revisions form input[type="checkbox"]').is(
      ":checked"
    );
    await axios
      .post(`/api/v1/service-report/checked/tally`, {
        id,
        data,
      })
      .then((res) => {
...
async function searchReport(query) {
  ...
  axios
    .post(`/api/v1/service-report/search/${query.replaceAll("/", "-")}`)
    .then((response) => {
      if (response.data.reports.length == 0) {
        ...
        totalSearchResults.html(
          `${response.data.reports.length} results found`
        );
      }
      $(".service-report-loading").addClass("hide");
      $(window).off("scroll");
...
$('form#advance-serach-form button[type="submit"]').on("click", (e) => {
  ...
  if (!$(".combination-adv-search-section").hasClass("opacity-3")) {
    ...
    axios({
      ...
      data: {
        wty: $(
          '#advance-serach-form .wty-adv-search-section input[name="advWty"]:checked'
        ).val(),
      },
    }).then(function (response) {
      ...
      for (var i = response.data.reports.length - 1; i >= 0; i--) {
        response.data.reports[i].service.forEach((machine) => {
          if (machine.warranty == response.data.query) {
            data += `
                        <tr class='cursor-pointer move service-report-row row-${i}' onclick='viewServiceReport(${JSON.stringify(
              response.data.reports[i]
            )});'>
                        <td class="user-select-none text-truncate" data-label="Machine name" >${
                          machine.machineName
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Machine number" >${
                          machine.machineNum
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Warranty" >${
                          machine.warranty
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Problem" >${
                          machine.problem
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Action" >${
                          machine.actionTaken
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Parts IN" >${
                          machine.partsIN.length
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Parts OUT" >${
                          machine.partsOUT.length
                        }</td>
                        <td class="user-select-none text-truncate" data-label="Status" >${
                          machine.status
                        }</td>
                        </tr>
                        `;
...
async function toggleServiceReportInputs(form, value) {
  if (form == "report") {
    ...
    $(".service-report-machine-details-machine-password").attr(
      "readonly",
      value
    );
...
async function saveServiceReport() {
  axios
    .post(
      `/api/v1/service-report/update/report/${$(
        ".service-report-details-_id"
      ).val()}`,
      {
...
async function saveReportMachinesDetails() {
  ...
  function createPartTable(part) {
    let parts = [];
    let PartName = $(`.parts-${part}-inputs .partName`)
      .map(function () {
        return this.value;
      })
      .get();
    let tempPartSno = $(`.parts-${part}-inputs .partSno`)
      .map(function () {
        return this.value;
      })
      .get();
    let tempPartWty = $(`.parts-${part}-inputs .partWty`)
      .map(function () {
        return this.value;
      })
      .get();
  ...
  axios
    .post(
      `/api/v1/service-report/update/machine/${$(
        ".service-report-machine-details-_id"
      ).val()}`,
      {
        "service.$.machineName": $(
          ".service-report-machine-details-machine-name"
        ).val(),
        "service.$.machineNum": $(
          ".service-report-machine-details-machine-number"
        ).val(),
        "service.$.machinePassword": $(
          ".service-report-machine-details-machine-password"
        ).val(),
        "service.$.warranty": $(".service-report-machine-details-wty").val(),
        "service.$.problem": $(".service-report-machine-details-prob").val(),
        "service.$.actionTaken": $(
          ".service-report-machine-details-action"
        ).val(),
    ...
    .then((response) => {
      if (response.data.success) {
        $(".submit-changes-service-report-machine-details-btn").addClass(
          "hide"
        );
